...
def test_hunt_with_users(hunt_client, **kwargs):
    ...
    for title in ['wg', 'ruhig', 'gruen', 'tausch', 'flat']:
        m.get(
            'https://api.telegram.org/bot1234xxx.12345/sendMessage?chat_id=1234&text=Great+flat+' + title + '+terrible+landlord',
            text=mock_response)
...
def test_multi_user_hunt_via_post(hunt_client, **kwargs):
    ...
    for title in ['wg', 'ruhig', 'gruen', 'tausch', 'flat']:
        m.get(
            ...
            text=mock_response)
        m.get(
            'https://api.telegram.org/bot1234xxx.12345/sendMessage?chat_id=1235&text=Great+flat+' + title + '+terrible+landlord',
            text=mock_response)
    app.config['HUNTER'].set_filters_for_user(1234, {})
...
def test_render_index_after_login(hunt_client):
    rv = hunt_client.get(
        '/login_with_telegram?id=1234&first_name=Jason&last_name=Bourne&username=mattdamon&photo_url=https%3A%2F%2Fi.example.com%2Fprofile.jpg&auth_date=123455678&hash=c691a55de4e28b341ccd0b793d4ca17f09f6c87b28f8a893621df81475c25952')
...
def test_login_with_telegram(hunt_client):
    ...
    assert json.dumps(session[
                          'user']) == '{"id": "1234", "first_name": "Jason", "last_name": "Bourne", "username": "mattdamon", "photo_url": "https://i.example.com/profile.jpg", "auth_date": "123455678"}'


def test_login_with_invalid_url(hunt_client):
    rv = hunt_client.get(
        '/login_with_telegram?username=mattdamon&id=1234&first_name=Jason&last_name=Bourne&photo_url=https%3A%2F%2Fi.example.com%2Fprofile.jpg&auth_date=123455678')
    assert rv.status_code == 302
    assert rv.headers['location'] == 'http://localhost/'
    assert 'user' not in session
...
def test_login_with_missing_params(hunt_client):
    rv = hunt_client.get(
        '/login_with_telegram?ad=1234&hash=51d737e1a3ba0821359955a36d3671f2957b5a8f1f32f9a133ce95836c44a9a9')
    assert rv.status_code == 302
    assert rv.headers['location'] == 'http://localhost/'
    assert 'user' not in session
...
def test_login_with_invalid_hash(hunt_client):
    rv = hunt_client.get(
        '/login_with_telegram?id=1234&first_name=Jason&last_name=Bourne&username=mattdamon&photo_url=https%3A%2F%2Fi.example.com%2Fprofile.jpg&auth_date=123455678&hash=0091a55de4e28b341ccd0b793d4ca17f09f6c87b28f8a893621df81475c25900')
    assert rv.status_code == 302
    assert rv.headers['location'] == 'http://localhost/'
    assert 'user' not in session
